...
int main(int argc, char**argv)
{
  ...
  //Add mnist data-set 
float IMnist [784],temp;
int k=0;

//for (int k=0; k<2;k++){ //k should be 59900 instead of 60000
int ** mnarray=readMnist();
       	for (int i=0;i<28;i++)
	   for (int j=0;j<28;j++)
		{
		//printf("%d ", mnarray[i][j]);
		temp =mnarray[i][j];
		IMnist [k]=temp/255;
		//printf("%f ", IMnist [k]);
		k++;
  ...
  // activation potential plot data for single neuron
	  if (i==20)
		  {
		    if (v <= -65)
     			vTemp=-65;
 ...
 /* //add for raster file
  FILE *output_file;
  output_file=fopen("nSpikes.csv", "w");


    for (int i;i<net.numNodes;i++)
 	 {
		Neuron* n = &neuron[i];		 
		fprintf(output_file,"%d ",i);
	 for (int j=0;j<100;j++)
	 //	printf("%d",spike[j]);
	     fprintf(output_file,"%d ",neuron[i].spikeTime [j]);  
	    fprintf(output_file,"\n");
